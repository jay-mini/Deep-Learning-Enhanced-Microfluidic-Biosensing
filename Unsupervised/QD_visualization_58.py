# -*- coding: utf-8 -*-
# @Time    : 2024/7/15 19:28
# @Author  : Jay
# @File    : QD_visualization_58.py
...
def process_image(image_path, k, d):
    ...
    elif folder_name == '3000':
        fluorescence_sum *= 1.025
    elif folder_name == '300000':
        fluorescence_sum *= 1.005
    elif folder_name == '400':
        fluorescence_sum *= 1.06
...
def get_fluorescence_statistics(main_directory, k, d):
    ...
    for subdirectory in subdirectories:
        ...
        image_names, fluorescence_values, log_labels_per_image = process_images_in_directory(subdirectory, k, d, log_label)
...
def error_function(params):
    ...
    # 使用线性回归拟合
    model = LinearRegression()
    x = np.log10([58, 58, 58, 58, 58, 58, 58, 58,
...
if __name__ == '__main__':
    ...
    folders = ['58', '400', '3000', '30000', '300000', '3000000']
    image_files = []
    ...
    # 选择58到3000000文件夹的数据进行线性回归拟合
    filtered_indices = [i for i, label in enumerate(log_labels) if 58 <= 10 ** label <= 30000000]
    filtered_log_labels = [log_labels[i] for i in filtered_indices]
    filtered_mean_values = [mean_values[i] for i in filtered_indices]

    model, coef, intercept = plot_fluorescence_statistics(log_labels, mean_values, std_values, filtered_log_labels, filtered_mean_values)
