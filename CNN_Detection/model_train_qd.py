...
if __name__ == '__main__':
    ...
    # 划分训练集和测试集
    X_train, X_test, y_train, y_test, train_categories, test_categories = train_test_split(images, labels,
                                                                                           categories_labels,
                                                                                           test_size=0.2,
                                                                                           random_state=44)
    # 数据转换
    ...
    markers = {'PBS-Buffer': 'o', 'Milk': '*', 'Chicken': 's'}

    # 训练集可视化
    plt.subplot(1, 2, 1)
    for cat in categories:
        indices = [i for i, c in enumerate(train_categories) if c == cat]
        plt.scatter(train_actuals[indices], train_predictions[indices], c=colors[cat], label=f'Train {cat}', marker=markers[cat])
    ...
    for cat in categories:
        ...
        plt.scatter(test_actuals[indices], test_predictions[indices], c=colors[cat], label=f'Test {cat}', marker=markers[cat])
